...
public class ImageService : IImageService
{
  ...
  public ImageService()
  {
    ...
    // Prefer Arial when present; otherwise fall back to the first available system family.
    FontFamily family;

    if (!_fonts.TryGet("Arial", out family))
      // Attempt via SystemFonts, then pick any available family to avoid runtime exceptions.
      if (!SystemFonts.TryGet("Arial", out family))
      {
        family = SystemFonts.Collection.Families.Any()
          ? SystemFonts.Collection.Families.FirstOrDefault()
          : _fonts.Families.First();
      }
